<!-- views/uploadview/Ruptura.vue - Versão Atualizada -->
<template>
  <div class="upload-container">
    ...
    <div class="loja-info-card">
      ...
      <p class="loja-descricao">
        Envie sua planilha de auditoria de ruptura para a loja selecionada
      </p>
    </div>
    ...
    >
      ...
      <span v-else> 🚀 Processar Planilha de Ruptura </span>
    </button>
    ...
    <div v-if="resposta" class="resposta-container">
      ...
      <div v-else class="success-card">
        ...
        <div class="success-stats">
          ...
          <div class="stat-item" v-if="resposta.loja">
            <span class="stat-number">{{
              resposta.loja.codigo || resposta.loja
            }}</span>
        ...
        <div class="success-actions">
          <button @click="irParaRuptura" class="btn-action primary">
            ⚠️ Ver Relatório de Ruptura
          </button>
...
const enviarArquivo = async () => {
  ...
  try {
    ...
    const { data } = await axios.post(
      "http://localhost:3000/upload-ruptura",
      formData,
      {
        headers: {
    ...
    resposta.value = data;
    console.log("✅ Upload de ruptura realizado:", data);
  } catch (error) {
    console.error("❌ Erro no upload de ruptura:", error);
...
const irParaRuptura = () => {
  // Redirecionar para onde você quer mostrar os dados de ruptura
  router.push("/ranking/ruptura"); // ou router.push("/ruptura")
};
...
<style scoped>
/* Usar os mesmos estilos do Etiqueta.vue, mas com cores de ruptura */
.upload-container {
  max-width: 800px;
  margin: 0 auto;
  padding: 2rem;
...
.loja-info-card {
  background: linear-gradient(135deg, #48a7be 0%, #0f6db9 100%);
  color: white;
  padding: 1.5rem;
  border-radius: 12px;
  margin-bottom: 2rem;
