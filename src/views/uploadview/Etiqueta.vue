<!-- uploadview/Etiqueta.vue -->
<template>
  <div class="upload-container">
    <h2>Enviar Planilha de Auditoria de Etiquetas</h2>
    <div class="upload-box" @dragover.prevent @drop="handleDrop">
      <p>Arraste sua planilha de etiquetas aqui ou</p>
      <input
        type="file"
        @change="handleFile"
        accept=".xlsx, .xls, .csv"
    ...
    <div v-if="resposta" class="resposta-container">
      ...
      <div v-else>
        ...
        <div v-if="mostrarPrevia" class="data-preview">
          <pre>{{ JSON.stringify(resposta.preview, null, 2) }}</pre>
        </div>
      </div>
    </div>
  </div>
