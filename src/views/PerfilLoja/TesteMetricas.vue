...
const buscarDadosCorredores = async () => {
  try {
    ...
    if (!lojaStore.codigoLojaAtual) {
      throw new Error(
        "Nenhuma loja selecionada. Por favor, selecione uma loja."
      );
    }

    console.log(
      `🔍 Buscando desempenho por corredor para loja: ${lojaStore.codigoLojaAtual}`
    );

    // Buscar métricas de desempenho por corredor
    const response = await axios.get(
      "http://localhost:3000/api/metricas/loja-daily/classes-completas", // Usando o mesmo endpoint que MetricasSetor.vue
      {
        headers: {
    ...
    if (response.data) {
      ...
      dadosReais.value = {
        ...
        etiquetas: {
          ...
          resumo: {
            ...
            usuariosAtivos:
              response.data.etiquetas?.resumo?.usuariosAtivos || 0,
          },
        },
        rupturas: {
        ...
        presencas: {
          ...
          resumo: {
            ...
            usuariosAtivos:
              response.data.presencas?.resumo?.usuariosAtivos || 0,
          },
        },
      };
      ...
      const totalLocais =
        Object.keys(dadosReais.value.etiquetas.locaisLeitura).length +
        Object.keys(dadosReais.value.rupturas.locaisLeitura).length +
        Object.keys(dadosReais.value.presencas.locaisLeitura).length;

      if (totalLocais === 0) {
        console.log(
          "⚠️ Nenhum dado de locais encontrado, verifique se o backend tem os dados corretos"
        );
      }
    }
  } catch (error) {
    ...
    if (error.response?.status === 400) {
      erro.value =
        "Loja não selecionada ou inválida. Por favor, selecione uma loja.";
    } else if (error.response?.status === 404) {
      erro.value =
        "Nenhuma métrica encontrada para a loja e data especificadas.";
